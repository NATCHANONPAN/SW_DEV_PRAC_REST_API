...
exports.getAppointments = async (req,res,next) =>{
    ...
    try {
        ...
        console.log(error)
        return res.stauts(500).json({sucess:false,message:"Cannot find Appointment"})
    }
}
  ...
  try {
    ...
    if(existedAppointments.length >=3 && req.user.role !== 'admin'){
        return res
          ...
          .json({
            success: false,
            message: `The user with ID ${req.user.id} has already made up 3 appointments`,
          });
    }
...
exports.updateAppointment = async (req, res, next) => {
  try {
    ...
    appointment = await Appointment.findByIdAndUpdate(req.params.id,req.body,{new:true,runValidatiors:true})
    res.status(200).json({ success: true, data: appointment });
  } catch (err) {
    console.log(err);
    return res
